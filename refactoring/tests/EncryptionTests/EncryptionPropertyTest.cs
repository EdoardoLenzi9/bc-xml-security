...
namespace Org.BouncyCastle.Crypto.Xml.Tests
{
    public class EncryptionPropertyTest
    {
        ...
        public void ConstructorWithXmlElement_InvalidXml()
        {
            ...
            XmlDocument doc = new XmlDocument();
            doc.LoadXml(xml);
            Assert.Throws<System.Security.Cryptography.CryptographicException>(() => new EncryptionProperty(doc.DocumentElement));
        }
        ...
        public void PropertyElement_Set_InvalidXml()
        {
            ...
            doc.LoadXml(xml);
            EncryptionProperty encryptionProperty = new EncryptionProperty();
            Assert.Throws<System.Security.Cryptography.CryptographicException>(() => encryptionProperty.PropertyElement = doc.DocumentElement);
        }
        ...
        public void LoadXml_InvalidXml()
        {
            ...
            doc.LoadXml(xml);
            EncryptionProperty encryptionProperty = new EncryptionProperty();
            Assert.Throws<System.Security.Cryptography.CryptographicException>(() => encryptionProperty.LoadXml(doc.DocumentElement));
        }
